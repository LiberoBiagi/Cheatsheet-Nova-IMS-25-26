...
\begin{itemize}
    \item Can deal with many files
    \item Small files are used
    \item We can scale elastically up and down
    \item Cheap
\end{itemize}
...
\begin{itemize}
    \item Atomic replacement of full objects
    \item Lower throughput
    \item High latency
    \item Lack of query engine 
    \item Off premises
\end{itemize}
...
\subsection{Machine Learning in Spark}
...
\begin{itemize}
    \item It works on distributed DataFrames
    \item Supports both supervised and unsupervised machine learning
    \item Integrates with pipelines
\end{itemize}
...
\begin{itemize}
    \item Distributed training on clusters
    \item Compatible with Spark SQL and DataFrames
    \item Integration with streaming and GraphX
    \item Used in production
\end{itemize}

\subsubsection{Spark MLlib concepts}

\textbf{Transformers}

These are functions that transform DataFrames using fixed rules. They do not learn from the data and implement the transform(). We get input\ra output columns. These functions are usually used in preprocessing. Some examples are Tokenizer, VectorAssembler, HashingTF, StandardScalerModel.
...
Pipelines are chains of transformations and estimators used to standardize the ML workflows. For example preprocessing \ra model training \ra evaluation. 
...
\begin{itemize}
    \item StringIndexer \ra Categorical encoding
    \item VectorAssembler \ra Combine features
    \item StandardScaler \ra Normalize data
\end{itemize}
...
\begin{itemize}
    \item BigDL \ra run DL models directly in Spark clusters
    \item Horovod \ra enables distributed training of TensorFlow and PyTorch models using Spark for data preprocessing and coordination.
\end{itemize}
...
\begin{itemize}
    \item Modular and reusable
    \item Automates repetitive steps
    \item Scales across distributed Spark clusters
    \item Encourage clean ML engineering practices
\end{itemize}


\subsubsection{Feature engineering pipelines}

Some useful pipelines can be

\begin{enumerate}
    \item \textbf{StringIndexer + OneHotEncoder} \ra To convert categorical values to numbers or to binary encode them 
    \item \textbf{VectorAssembler} \ra Combines multiple feature columns into a single one
    \item \textbf{StandardScaler} \ra Standardizes the feature into an N(0, 1) fashion to help our algorithms converging faster  
\end{enumerate}
...
\subsubsection{Advanced Pipelines}
...
\begin{itemize}
    \item Node (or vertex) \ra entity
    \item Edge \ra relationship between nodes
    \item Directed/Undirected, Weighted/Unweighted
\end{itemize}

We can apply graph analytics to do tasks like
\begin{itemize}
    \item Community detection
    \item Suspicious pattern detection
...
\begin{itemize}
    \item mapVertices/mapEdges \ra transform attributes
    \item joinVertices \ra enrich data
    \item subgraph \ra filter
    \item triplets \ra combine vertices + edges
\end{itemize}
...
\begin{itemize}
    ...
    \item The Shortest Paths
    \item Label propagation (communities)
\end{itemize}
...
\subsubsection{Algorithms in details}

Here we can see how our algorithms can work

\begin{itemize}
    \item PageRank \ra We consider a node important if many important nodes link to it. We iteratively distribute rank across the graph to find which is the most influential node. 
    \item Connected components \ra We group vertices connected by paths
    \item The Shortest path \ra We want to find the minimum distance path between nodes by applying algorithms like Dijkstra or BFS. 
    \item Triangle counting \ra We have a triangle when 3 nodes are all connected, this can tell us the clustering coefficient (how tight a network is).
\end{itemize}
...
\subsubsection{Structured streaming overview}
...
\subsubsection{Streaming concepts}

Some useful concepts about streaming

\begin{itemize}
    \item \textbf{Micro-Batch model} \ra Spark processes small batches of data, we want a balance between throughput and latency. This is transparent to developers
    \item \textbf{Event time vs Processing time} \ra Event (when the event occurs) or processing (when Spark processes it). Structured Streaming handles late data with watermarks
    \item \textbf{Windowing} \ra Data are grouped into time windows
    \item \textbf{Exactly-one semantics} \ra Spark guarantees no duplicate processing even when failures happen, this is important for finance and transactions. We have it when we have replayable source, deterministic transformations, idempotent or transactional sink and a stable checkpoint directory
    \item \textbf{Checkpointing and fault tolerance} \ra States are stored in checkpoints, this allows recovery after failure. Key for production streaming jobs
\end{itemize}


\subsubsection{Spark structured streaming API}
...
\begin{itemize}
    \item Writes only new rows to the result table since last trigger
    \item It requires that those rows will never change later
    \item Typical queries are stateless transforms and event-time window aggregations with watermark
    \item Good for append-only sinks like files or data lakes
\end{itemize}

Update
\begin{itemize}
    \item Writes only rows whose values changed since the last trigger
    \item Can output the same key multiple times as aggregates evolve
    \item Typical queries are stateful ones
    \item It's good for sinks that can handle upserts and overwrites
\end{itemize}

Complete
\begin{itemize}
    \item It writes the entire result table every trigger
    \item Only practical when the resultant table is small
    \item Typically used for debugging, dashboards or in-memory tables where a full refresh is cheap
    \item Not ideal for file sinks
\end{itemize}
...
\begin{enumerate}
    ...
    \item Transform the DataFrame
    \item Choose sink + output mode
    \item Configure trigger + checkpoint
    \item Start and monitor the query
\end{enumerate}




\subsubsection{Advanced streaming}
...
\subsubsection{Challenges and best practices}

Some challenges in Streaming can be
\begin{itemize}
    \item Out-of-order events
    \item Late arrivals
    \item Fault tolerance at scale
    \item Cost of always-on clusters
\end{itemize}

Best practices
\begin{itemize}
    \item Use watermarks for late data
    \item Keep stateless when possible
    \item Monitor with Spark UI and logs
    \item Test with small replayable streams
\end{itemize}
...
\subsubsection{Deep dives}

In more details

\begin{itemize}
    \item Windowed aggregation \ra We can have a fixed window, a sliding window or a session window
    \item Watermarks for late data \ra To handle delayed events, if an event is later than the selected time frame it will be ignored. 
    \item Stream joins \ra Can be stream-to-stream or stream-to-batch
\end{itemize}

\subsection{The future of Big Data}
...
\subsubsection{Current limitations of Big Data}
...
\begin{itemize}
    \item Shift from on-prem Hadoop to cloud
    \item Data lakehouse are being used much more (scalability of lakes + governance of warehouse)
    \item Shift from batch to real time insights
    \item Lower complexity for unified batch and streaming
    \item The need for serverless Big Data
\end{itemize}

\subsubsection{AI and Big Data integration}
...
\subsubsection{Privacy, ethics and regulation} 
...
\begin{itemize}
    \item Bias in AI pipelines
    \item Surveillance risks
    \item The need for explainability
\end{itemize}
...
\subsubsection{Semantic data and interoperability}
...
\subsubsection{MLOps, DataOps, and AIOps convergence}

In the future we will have much more automation across the lifecycle of our analytics

\begin{itemize}
    \item DataOps \ra Automates data ingestion, quality and lineage
    \item MLOps \ra Automates model training, validation and deployment
    \item AIOps \ra Uses AI to optimize and monitor data and ML pipelines
\end{itemize}

\subsubsection{Research and open challenges}

We still need to solve some problems like:

\begin{itemize}
    \item Scalability challenges
    \item Processing at edge
