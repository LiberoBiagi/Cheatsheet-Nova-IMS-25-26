...
\subsection{Introduction to Text mining}
\subsubsection{Introduction to NLP}
...
\begin{itemize}
    \item \textbf{Curse of dimensionality} \ra The total dimension of the BoW is the vocabulary size. Our model can easily overfit. We can use dimensionality reduction techniques.
...
\begin{itemize}
    ...
    \item Calculate context probability
...
\begin{enumerate}
    ...
    \item Define the task, usually if word w and c appeared in the corpus. Use the weights of our first model to represent the words
    \item Train the model to get our embedding size and get the probabilities for each possible word
...
\begin{itemize}
    \item Compute how much to focus on each encoder state. Relevancy is captured by computing for each state a score measure. This score is called dot product attention $\text{score} (h_{i-1}^d, h_j^e ) = h_{i-1}^d \cdot h_j^e$ 
    \item The score that we get is a scalar that reflects the degree of similarity between two vectors. To make use of these scores we have to normalize them with a softmax to create a vector of weights. This vector will tell us the proportional relevance of each encoder hidden state to the prior hidden decoder state $\alpha_{ij} = \text{softmax} ( \text{score}(h_{i-1}^d, h_{j}^e) \forall j \in e)$
    \item Finally we compute a fixed length context vector for the current decoder state by taking a weighted average over all the encoder hidden states $c_i = \sum_{j}\alpha_{ij} h_{j}^e$
\end{itemize}
...
\begin{itemize}
    \item Tokenize the input sequence using the tokenizer of the transformer model
...
These summed vectors are then normalized using layer normalization using a variant of the standard score by applying it to a single hidden layer. In the standard implementation we introduce two parameters called $\gamma$ and $\beta$ representing gain and offset values. 
...
\begin{itemize}
    \item Contextualized \ra Each word is represented based on its contextual meaning, allowing the same word to have different representations depending on surrounding words
    \item Bidirectional \ra Representations are influenced by both left and right context
    \item Position-aware \ra Using positional embedding the model retains information about word order
\end{itemize}
...
\begin{enumerate}
    \item Sample a word in the output to get our first input
    \item Use the word embedding for that first word as the input to the network at the next time step
    \item Continue generating until the end marker is reached
\end{enumerate}

\subsubsection{Generative AI models}

Two main kinds of LLMs exist

\begin{itemize}
    \item Proprietary models \ra Also known as closed source, they can be accessed by an API, as a result the details of their code and architecture are not shred with the user
    \item Open models \ra they are directly managed by the user and so people can see code and architecture
\end{itemize}

As we know these kinds of models are huge and to reduce their weight and size we can try different strategies

\begin{itemize}
    \item Quantization \ra reduces model size and memory usage by reducing the number of bits needed to represent the parameters of an LLM. It enables them to run faster and with less powerful hardware requirements
    \item GGUF \ra GPT-Generated Unified Format, a file format used to represent a compressed version of the quantized LLM
\end{itemize}
...
\begin{itemize}
    \item Temperature \ra the randomness of the generated text. How easy is to select tokens that are less probable
    \item Top\_p \ra nucleus sampling, control which subset of tokens the LLM can consider
    \item Top\_k \ra how many tokens the LLM can consider  
\end{itemize}
...
\begin{itemize}
    \item Dense search \ra uses embedding to capture meaning and intent
    \item Keyword search \ra matches exact terms in documents while ignoring semantics
    \item Hybrid search \ra run keyword search and semantic one while combining the scores of both methods using a weighted formula
\end{itemize}
...
\begin{itemize}
    \item Memory \ra short term and long term
    \item Tools \ra APIs that enable real-time interaction with dynamic environments
    \item Planning/reasoning \ra The core generative capability of the LLM, which forms plans and generates responses
\end{itemize}
